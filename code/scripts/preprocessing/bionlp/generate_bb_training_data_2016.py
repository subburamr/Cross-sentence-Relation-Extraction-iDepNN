...
if __name__ == '__main__':
    ...
    if data_type == "NER+REL":
        ...
        annotation_dict = load_save_pkl.load_pickle_file(os.path.join(output_path,"BB2016_annotation_dict.pkl"))
        doc_list_dict = load_save_pkl.load_pickle_file(os.path.join(output_path, "BB2016_doc_list_dict.pkl"))
        ...
        data_dir = ["BioNLP-ST-2016_BB-event_" + dataset_type +"/"]#,
                    #"BioNLP-ST-2016_BB-event_dev/"]
        ...
        for dir in data_dir:
            ...
            for filename in list_filepaths:
                ...
                if filename.endswith(".a1"):
                    ...
                    for line in open(str(str(filename).split('.a1')[0])+".a2").readlines():
                        line = str(line).strip('\n')
        ...
        print(annotation_dict)
        # exit(0)
        load_save_pkl.save_as_pkl(annotation_dict, os.path.join(output_path, "BB2016_" + dataset_type + "_annotation_dict.pkl"))
        load_save_pkl.save_as_pkl(doc_list_dict, os.path.join(output_path, "BB2016_" + dataset_type + "_doc_list_dict.pkl"))
        # exit(0)
        print('star_count:', star_count)
        # exit(0)
        inter_sent_rel_count = 0
        ...
        for filename in annotation_dict.keys():
            ...
            for rel_key in rel_dict.keys():
                ...
                if k_val == 0:
                    ...
                    if start_e1_indx <= start_e2_indx <= end_e1_indx or start_e2_indx <= start_e1_indx <= end_e2_indx:
                        if start_e1_indx <= start_e2_indx <= end_e2_indx <= end_e1_indx:
                            ...
                            target_sent = target_sent.replace(e2_word, "<e2>" + e2_word + "</e2>", 1)
                            target_sent = target_sent.replace(e1_word, "<e1>" + e1_word + "</e1>", 1)
                        with open(os.path.join(output_path,"BB2016_" + dataset_type + "_containment_data.txt"),'a') as containment_data_file:
                            containment_data_file.write(str(rel_id) + "::" + str(filename.split("/")[-1])  + "::" + str(k_val) + "::" + str(relation)  + "::" + str(e1_word)  + "::" + str(e2_word) + "::" + target_sent + "::" + e1_type + "::" + e2_type +  "::" + relation_e1 +  "::" + relation_e2 +  "::" + rel_key)
                ...
                tags = ['<e1>','</e1>','<e2>','</e2>']
                if not all(x in target_sent for x in tags):
                    with open(os.path.join(output_path,"BB2016_" + dataset_type + "_error_data.txt"),'a') as error_data_file:
                ...
                with open(os.path.join(output_path,"BB2016_" + dataset_type + "_train_data.txt"),'a') as data_file:
                    ...
                    data_file.write('\n')
        load_save_pkl.save_as_pkl(target_sent_list_dict, os.path.join(output_path,"BB2016_" + dataset_type + "_train_data_target_sent_list_dict.pkl"))
